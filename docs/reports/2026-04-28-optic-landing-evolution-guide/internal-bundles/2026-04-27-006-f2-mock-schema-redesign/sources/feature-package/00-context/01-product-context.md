...
| 지표 2 — AI_EXTRACT 이전 완성 수치 미노출 | CompanyManager 외 전체 추출 대상 placeholder/neutral 처리 |
| 지표 3 — 3개 이상 시나리오 세트 순환 | randomizable demo-safe scenario 3개 + loop-start random rotation |
...
- `AI_APPLY` 전에는 CompanyManager를 제외한 상/하차지, 일시, 화물, 옵션, estimate, settlement 완성값이 먼저 보이지 않는다.
- `AI_APPLY`에서는 상차지 → 하차지 → 예상 운임/거리 → 화물/차량+운송 옵션 → 정산 정보 순서로 적용된다.
- preview loop마다 `default`, `regional-cold-chain`, `short-industrial-hop` 중 하나가 선택되어 반복 demo가 단조롭지 않다.
- `partial`, `mismatch-risk` scenario는 테스트 fixture로만 유지된다.

## 5. 구현 결과

| 항목 | 결과 |
|---|---|
| 상태 | implemented → archived 준비 완료 |
| 구현일 | 2026-04-27 |
| 검증 | `pnpm test` 45 files / 1039 tests, `typecheck`, `lint`, `build` PASS |
| archive 대상 | IDEA, Screening, Draft/PRD/Review, Feature Package, Dev Notes |
