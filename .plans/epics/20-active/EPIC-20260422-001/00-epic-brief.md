...
- **지표 2 — AI_EXTRACT 이전 단계에서 완성 수치 미노출**: `EstimateInfoCard`·`SettlementSection` 이 Step 미만일 때 placeholder 또는 미렌더. 스냅샷/스텝별 가시성 테스트 추가.
- **지표 3 — 3 개 이상 시나리오 세트 순환**: `PREVIEW_MOCK_SCENARIOS` 배열에 최소 3 세트 등록 + 선택기 함수 동작. 재방문 시 서로 다른 세트 노출 (트리거 방식은 F2 에서 결정).
...
- **F2 — Mock 스키마 재설계 (추출/적용 분리 + 시나리오 세트)** (IDEA-{미정})
  - 범위: `PREVIEW_MOCK_DATA` 단일 상수를 `extractedFrame`/`appliedFrame` 분리 구조 + `PREVIEW_MOCK_SCENARIOS` 배열로 전환. `EstimateInfoCard`/`SettlementSection` Step 기반 가시성 게이트. 선택기 함수 (세트 샘플링) 도입. 이슈 [2-1], [2-2], [2-3], [2-4].

- **F3 — 옵션↔추가요금 파생 로직** (IDEA-{미정})
  - 범위: `OPTION_FEE_MAP` 도입, `options` 변화에 따라 `additionalFees` 파생. `settlement-section` 소스 전환. 이슈 [2-5] 단일.

- **F4 — 레이아웃 정비 + Hit-Area 재정렬** (IDEA-{미정})
...
- [IDEA-20260423-002 — F1 라이트 모드 전환 인프라](../../../ideas/00-inbox/IDEA-20260423-002.md)
- IDEA-{미정} — F2 Mock 스키마 재설계 (추출/적용 분리 + 시나리오 세트)
- IDEA-{미정} — F3 옵션↔추가요금 파생 로직
- IDEA-{미정} — F4 레이아웃 정비 + Hit-Area 재정렬
- [IDEA-20260423-001 — F5 UI 잔재 정리 (JSON 뷰어 + 라벨)](../../../ideas/00-inbox/IDEA-20260423-001.md)
