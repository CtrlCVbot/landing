...
- **F2 — 카피와 제품 라인업 정리**
  - `Cargo` 중심 언어를 줄이고 `오더`, `운송`, `조율`, `정산` 중심 문구로 전환. 제품 라인업은 `OPTIC Broker/Shipper`를 현재 구현 대상으로 두고 `Carrier/Ops/Billing`은 구현 예정으로 분리.
