...
| Features section | `주문 관리`, `세금계산서`, `AI 주문 추출`, provider명 포함 설명 | AI 오더 등록, 배차 단계의 화물맨 연동, 정산 자동화, 세금계산서 관리 중심 |
| Integrations section | `Google Gemini AI`, `카카오 맵`, `팝빌`, `로지스엠/화물맨` | `화물맨` 외 provider명은 일반 기능명으로 낮추고, 화물맨은 배차 정보 전송/중복 등록 감소 문맥으로 보조 |
...
- `화물맨 연동`은 Features 섹션에서 배차 단계 기능으로 추가하고, Integrations 섹션에서는 provider 설명보다 전송 누락/중복 등록 감소 효과를 보조 설명한다.
