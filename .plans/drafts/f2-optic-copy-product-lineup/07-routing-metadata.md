...
| Hybrid | `false` |
| PRD required | `true` |
| PRD draft | [f2-optic-copy-product-lineup-prd.md](../../prd/00-draft/f2-optic-copy-product-lineup-prd.md) |
...
| Dev verify | `pending` |
| Archive | `pending` |
| Recommended next | `/plan-review .plans/prd/00-draft/f2-optic-copy-product-lineup-prd.md --type=prd` |
...
```json
{
  ...
  "prd_required": true,
  "prd": ".plans/prd/00-draft/f2-optic-copy-product-lineup-prd.md",
...
**Standard + dev + hybrid=false** → `/plan-prd` 필수. PRD draft는 생성 완료 상태다.

F2는 텍스트와 상수 정리 중심이므로 wireframe/stitch는 기본적으로 생략 후보지만, PRD에서 섹션 구조나 카드 배치를 크게 바꾸는 것으로 확정되면 P5/P6를 재평가한다.

```bash
/plan-review .plans/prd/00-draft/f2-optic-copy-product-lineup-prd.md --type=prd
```
