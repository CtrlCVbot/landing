...
| Dev verify | `pending` |
| Archive | `pending` |
| Recommended next | `/plan-bridge f2-optic-copy-product-lineup` |
...
```json
{
  ...
  "triggers_matched": [1],
  "entryPoint": "P4-plan-prd-review",
  "next": "/plan-bridge f2-optic-copy-product-lineup",
...
**Standard + dev + hybrid=false** → `/plan-prd` 필수. PRD review 결과 `Approve`로 통과했으며, F2는 기존 섹션의 copy/data 정리이므로 wireframe/stitch는 생략한다.

F2는 텍스트와 상수 정리 중심이므로 wireframe/stitch는 기본적으로 생략 후보지만, PRD에서 섹션 구조나 카드 배치를 크게 바꾸는 것으로 확정되면 P5/P6를 재평가한다.

```bash
/plan-bridge f2-optic-copy-product-lineup
```
...
| 2026-04-29 | PRD review 통과 — Broker/Shipper 2개 구현 대상 기준 반영, wireframe/stitch 생략, 다음 단계 `/plan-bridge` |
