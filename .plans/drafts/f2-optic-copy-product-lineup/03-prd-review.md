...
| 제품 라인업 범위 | PASS after fix | 사용자 피드백에 따라 현재 구현 대상은 `OPTIC Broker`, `OPTIC Shipper` 2개로 제한하고 `Carrier/Ops/Billing`은 구현 예정으로 분리 |
...
| 용어 일관성 | 완료 | `OPTIC`, `OPTICS`, `화물맨`, Broker/Shipper 구현 대상 기준 정리 |
...
| PCC-04 PRD ↔ Wireframe | SKIP | F2는 기존 섹션의 copy/data 정리이며 신규 화면 구조를 만들지 않음 |
| PCC-05 Wireframe ↔ Stitch | SKIP | Wireframe 생략에 따라 Stitch도 생략 |
...
|---|---|---:|---|---|---|
| 제품 라인업이 5개 활성 제품처럼 해석될 수 있음 | high | 5 | confirmed | auto-fixed | Broker/Shipper 2개 구현 대상, Carrier/Ops/Billing 구현 예정으로 PRD/Draft/IDEA/Epic 반영 |
| wireframe/stitch 조건이 `conditional`로 남아 다음 단계가 흔들릴 수 있음 | medium | 4 | confirmed | auto-fixed | routing metadata에서 wireframe/stitch 생략과 `/plan-bridge` 다음 단계를 명시 |
...
| P5/P6 routing ambiguity | 2 | 1 | 1 | 4 | confirmed | auto-fixed |
...
PRD는 F2 구현에 필요한 요구사항과 검증 기준을 충분히 담고 있다. 사용자 피드백으로 제품 라인업 범위를 조정해, 현재 구현은 `OPTIC Broker`, `OPTIC Shipper` 두 가지에 집중하고 `OPTIC Carrier`, `OPTIC Ops`, `OPTIC Billing`은 구현 예정으로 분리했다.

F2는 신규 레이아웃이나 새로운 화면 흐름을 만드는 작업이 아니라 기존 섹션의 copy/data 정리다. 따라서 `/plan-wireframe`과 `/plan-stitch`는 생략하고, 다음 단계는 `/plan-bridge f2-optic-copy-product-lineup`로 진행한다.

## 6. Next Steps

1. `/plan-bridge f2-optic-copy-product-lineup`로 개발 핸드오프 문서를 만든다.
2. Bridge에서 구현 대상 제품을 `OPTIC Broker`, `OPTIC Shipper`로 고정하고, `Carrier/Ops/Billing`은 구현 예정 표시 기준으로 넘긴다.
3. Dev package에서 금지 문구, 제품명, 외부 브랜드명, 커스텀 메시지, CTA 문구 스캔을 acceptance criteria로 유지한다.
