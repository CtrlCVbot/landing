...
- **R8**: `src/components/sections/features.tsx`는 `주문 관리` 중심 표현을 `AI 오더 등록`, 배차 단계의 `화물맨 연동`, 정산 자동화, 세금계산서 관리 흐름과 연결한다.
...
| 4. 배차 단계의 화물맨 연동 | 한 번 입력한 운송 정보를 배차 단계에서 외부 채널로 연결 | 중복 등록과 전송 누락 감소 |
...
| 기능 섹션 | `src/components/sections/features.tsx` | AI 오더 등록, 배차 단계의 화물맨 연동, 정산 자동화, 세금계산서 관리 중심으로 정렬 |
...
| 2026-04-29 | 사용자 피드백 반영 — Features에 배차 단계의 화물맨 연동 기준 추가 |
