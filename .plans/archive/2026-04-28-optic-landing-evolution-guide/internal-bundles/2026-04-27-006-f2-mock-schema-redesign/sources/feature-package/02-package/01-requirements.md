...
| ID | 우선순위 | 요구사항 | 수용 기준 |
|---|:---:|---|---|
| REQ-f2-mock-schema-001 | Must | `PREVIEW_MOCK_SCENARIOS` 배열을 도입한다. | 최소 5개 scenario가 존재하며, demo preview에서 순환 가능한 scenario는 3개 이상이다. |
...
| REQ-f2-mock-schema-004 | Must | scenario selector helper를 제공한다. | deterministic helper와 random helper가 모두 있고, random helper는 `excludeId`로 직전 scenario 연속 선택을 피한다. |
...
| REQ-f2-mock-schema-007 | Must | `AI_APPLY` 이전에는 AI 추출 대상 값이 노출되지 않는다. | CompanyManager는 pre-filled로 유지하되 상/하차지, 일시, 화물, 옵션, estimate, settlement는 placeholder/neutral 상태로 보인다. |
...
| REQ-f2-mock-schema-011 | Must | `AI_APPLY` 내부에 파트별 적용 타임라인을 둔다. | 상차지 → 하차지 → 예상 운임/거리 → 화물/차량+운송 옵션 → 정산 정보 순서로 표시된다. |
| REQ-f2-mock-schema-012 | Must | `mismatch-risk`는 preview random pool에서 제외한다. | `randomizable=false`이며 테스트 fixture로만 선택 가능하다. |
...
| ID | 요구사항 | 수용 기준 |
|---|---|---|
| NFR-f2-001 | preview loop마다 demo-safe scenario 다양성 확보 | 마지막 Step에서 Step 1로 돌아올 때 randomizable scenario가 교체되고 같은 scenario가 연속 선택되지 않는다. |
...
| NFR-f2-006 | 애니메이션 판독성 | Step duration과 하위 animation이 이전보다 느려져 사용자가 파트별 적용 순서를 눈으로 확인할 수 있다. |
...
| `jsonViewerOpen` 처리 | `extractedFrame.aiResult.jsonViewerOpen=false` 유지 | `00-context/02-decision-log.md` |
| `mismatch-risk` 노출 범위 | 테스트 fixture only, random preview 제외 | `00-context/02-decision-log.md` |
| 파트별 reveal timing | `formRevealTimeline` 기준으로 `AI_APPLY` 내부 세분화 | `00-context/02-decision-log.md` |
