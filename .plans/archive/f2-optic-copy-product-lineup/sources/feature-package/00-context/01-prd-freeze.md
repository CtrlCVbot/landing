...
| 포함 | Features 섹션에 배차 단계의 `화물맨 연동` 기준 반영 |
...
| `US-F2-002` | 주선사 운영자가 오더-배차-화물맨 연동-정산 흐름을 업무 흐름으로 이해한다. | `REQ-F2-005`, `REQ-F2-006`, `REQ-F2-009`, `REQ-F2-011` |
...
| `AC-F2-009` | Features 섹션에서 `화물맨 연동`은 배차 단계 기능으로 보인다. |
| `AC-F2-010` | 375px 모바일 폭에서 제품 카드와 버튼 텍스트가 겹치지 않는다. |
| `AC-F2-011` | F3가 이어받을 업무 단계 흐름이 문서 또는 후속 package에 추적 가능하다. |
