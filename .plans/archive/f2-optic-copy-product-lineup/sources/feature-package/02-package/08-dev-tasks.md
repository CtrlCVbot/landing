...
| TASK | 제목 | 상태 | 주요 파일 | 검증 |
|---|---|:---:|---|---|
| T-F2-01 | constants copy/data 정리 | done | `src/lib/constants.ts`, `src/__tests__/lib/constants.test.ts` | targeted test |
| T-F2-02 | Features/Problems/Integrations 카피 정리 | done | `features.tsx`, `problems.tsx`, `integrations.tsx`, section tests | targeted test |
| T-F2-03 | Products 섹션 구조 정리 | done | `products.tsx`, `constants.ts`, `products.test.tsx` | targeted test |
| T-F2-04 | Hero/Header/Footer 회귀 보정 | done | `hero.tsx`, `header.tsx`, `footer.tsx`, existing tests | targeted test |
| T-F2-05 | 검증, 문구 스캔, preview evidence | done | test output, browser evidence | full checks |
...
## 6. T-F2-05 - 검증, 문구 스캔, preview evidence
...
- [x] Local preview server `http://localhost:3102/` returned `200` and was opened.
- [ ] Responsive viewport visual check 1440px/768px/375px in `/dev-verify`.
- [ ] Keyboard focus visible check in `/dev-verify`.
