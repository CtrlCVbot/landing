...
- 제품 라인업을 `OPTIC Broker`, `OPTIC Shipper`, `OPTIC Carrier`, `OPTIC Ops`, `OPTIC Billing` 기준으로 정렬한다.
...
| REQ-f2-optic-copy-product-lineup-007 | 제품 라인업을 `OPTIC Broker/Shipper/Carrier/Ops/Billing`으로 정렬한다. | Must | `OPTIC Operations`는 `OPTIC Ops`로 정리되고, 5개 제품명이 문서 기준과 일치한다. |
| REQ-f2-optic-copy-product-lineup-008 | 제품 설명은 역할별 업무 가치와 커스텀 가능성을 연결한다. | Must | Broker는 주선사 운영/정산, Shipper는 요청/현황, Ops는 다중 고객 운영 조율, Billing은 정산/증빙 흐름을 설명한다. |
...
- 제품 라인업은 역할별 선택지를 빠르게 훑을 수 있어야 한다. 각 제품 카피는 대상 사용자와 대표 업무를 한 문장 안에서 연결한다.
...
- 현재 `src/lib/constants.ts`에는 `OPTIC Operations`, `Google Gemini AI`, `카카오 맵`, `팝빌`, `로지스엠/화물맨`, `주문 관리`, `AI 주문 추출` 등이 있어 F2 스캔 기준으로 삼는다.
...
| M4 — Section render update | 관련 섹션의 표시 문구 정리 | 제품 라인업, 커스텀 메시지, 업무 가치 문구 표시 |
...
| SM-f2-copy-product-lineup-002 | 제품명 정합성 | 5개 제품명 모두 `OPTIC Broker/Shipper/Carrier/Ops/Billing` 기준 | 제품명 스캔 또는 constants test |
