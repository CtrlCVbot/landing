...
- 이번 구현 대상 제품 라인업은 `OPTIC Broker`, `OPTIC Shipper` 두 가지로 고정한다.
...
| REQ-f2-optic-copy-product-lineup-007 | 현재 구현 제품 라인업은 `OPTIC Broker`와 `OPTIC Shipper` 두 가지로 정렬한다. | Must | 고객-facing 제품 섹션에서 구현 대상 제품은 Broker/Shipper로 보이며, `OPTIC Operations`는 활성 제품명으로 남지 않는다. |
...
- 제품 라인업은 현재 구현 대상과 구현 예정 대상을 혼동 없이 구분해야 한다. 활성 제품 카피는 `OPTIC Broker`와 `OPTIC Shipper`에 집중하고, 나머지는 로드맵 성격으로 낮춘다.
...
- 현재 `src/lib/constants.ts`에는 `OPTIC Operations`, `Google Gemini AI`, `카카오 맵`, `팝빌`, `로지스엠/화물맨`, `주문 관리`, `AI 주문 추출` 등이 있어 F2 스캔 기준으로 삼는다. 특히 제품 라인업 구현 범위는 `OPTIC Broker`와 `OPTIC Shipper` 두 가지로 제한한다.
...
| M4 — Section render update | 관련 섹션의 표시 문구 정리 | Broker/Shipper 중심 제품 라인업, 구현 예정 제품 분리, 커스텀 메시지, 업무 가치 문구 표시 |
...
| SM-f2-copy-product-lineup-002 | 제품명 정합성 | 구현 대상은 `OPTIC Broker`, `OPTIC Shipper` 2개로 보이고, `OPTIC Carrier/Ops/Billing`은 구현 예정으로만 구분 | 제품명 스캔 또는 constants test |
